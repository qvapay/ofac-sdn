...
- `scripts/import-lists.mjs` runs the scrapers in `scripts/sources/` (`pcc.mjs` — pcc.cu Drupal table, paginated `?page=N`; `anpp.mjs` — parlamentocubano.gob.cu, all deputies on one page; `fhrc.mjs` — represorescubanosfhrc.com, a SPA whose data lives in a public Supabase table: the module extracts the Supabase URL + anon key from the deployed JS bundle at run time and pages through the REST API; `un.mjs` / `eu.mjs` / `uk.mjs` — official sanctions XML downloads parsed with fast-xml-parser: UN Security Council consolidated list, EU FSF consolidated list (public-token URL, override via `EU_FSF_URL`), UK OFSI consolidated list (~55 MB, rows grouped by `GroupID` into entities; refreshed daily by `.github/workflows/sanctions-import.yml`); `opensanctions.mjs` — OpenSanctions consolidated `targets.simple.csv` (~64 MB, daily), filtered to entities NOT already on a directly-imported list (drops any row whose `dataset` labels include OFAC SDN, UN, EU FSF, or UK FCDO — and throws if a covered label matches 0 rows, i.e. OpenSanctions renamed it); nets ~44k extra entities from ~80 sources (Canada, Switzerland, Australia, Japan, US CSL/BIS, Israel crypto wallets…). License is CC-BY-NC: OK for this free public service, but keep "Data: OpenSanctions" attribution visible in the UI, and it is deliberately NOT in the daily workflow yet — add `opensanctions` to the `sanctions-import.yml` run line to ship it). Each source exports `{ id, label, source, fetchEntities }` and returns entities in the same shape the runtime consumes (`type: 'Individual'`, `names: [{ full, isPrimary }]`, plus `role`/`org`/`organizations`/`url` extras that pass through to responses). With `--upload` it pushes `lists/<id>.json` to R2 and upserts `manifest.json` (seeding an `ofac` entry pointing at `R2_KEY`). It refuses to write a list that scraped 0 entities (site-redesign guard) unless the source exports `allowEmpty = true`.
